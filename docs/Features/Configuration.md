...
`hourly`, `monthly`, `weekly`, or a [raw crontab expression](#advanced-crontab-expressions).
...
- Filesystem (default, for single instance deploy)
- [Windows Azure](#windows-azure)
- [Amazon AWS](#amazon-aws)
- [Dropbox](#dropbox)

Remember to deploy your application with the following command to apply any
change.
...
Anatomy of a `security.yaml` file:

- [allowed_origins](#allowed_origins): list of domains that are allowed to use JavaScript client as referrer.
- [roles](#roles): list of available auth roles for the application.
- [collections](#collections): which auth role can do `read`, `create`, `update` and `delete` operations
...
There is two built-in roles are `all` and `owner`.
...
The allowed arguments for each operations are `all`, `owner` and any roles you've defined in the previous section.
...
They'll be installed in the server after running `hook deploy`.
